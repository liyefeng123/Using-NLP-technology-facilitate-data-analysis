...
with open('./Msc-twitter-data/question_classifier/data/config.txt', 'r') as file1:
    idx = 0
    config_title = []  #配置项目名称
...
def test_pre(model,label_dictionary,word2idx,vocab):
    ...
    for i in range(len(test_data)):
        ...
        if np.argmax(pre_tag) == test_label[i]:
            right_num += 1
    y_pred.append(np.argmax(pre_tag))
    ...
    for i in range(len(output)):
        ...
        file.write('\n');
    file.close()
    # cm = confusion_matrix(test_label, y_pred, list(range(0, 2)))
    ...
    # temp = []
    # Inf = 1000000000
    # list_recall = recall.tolist()
    # for i in range(5):
    #     temp.append(list_recall.index(min(list_recall)))
    #     list_recall[list_recall.index(min(list_recall))] = Inf
    # print(temp)
    # for i in temp:
    ...
    #     f.save(path)
    #
    # saves(cm.astype(np.float64), r"./data/a.xls")
    # label_dictionary = [doc.encode('latin-1', 'ignore') for doc in label_dictionary]
    # # convert list to array
    # docs_array = np.array(label_dictionary)
    # print(type(docs_array))
    # # saving...
    # np.savetxt(r"./data/b.csv", docs_array, fmt='%s', delimiter=',')
############################################随机的测试###########################################
